...
2026-09-07：用户已授权推送与发布两项修复，准备以 v2.19.2 正常快进 main；版本及资源查询统一更新，后台与词库不变。下方“尚未推送”是前一轮交接记录，发布结果待本次生产回读更新。

2026-09-06 自检后两项修复在 `codex/protect-progress-and-streak` 本地完成，尚未推送/发布，线上仍为下述 v2.19.1：连续天数按日期幂等记账，覆盖已置 completed 的最后一题、刷新恢复且不计空任务；多页面通过 `${STORAGE_KEY}:writer` Web Locks 独占写入，读取旧快照的页面不覆盖新记录。锁随关闭/离开释放，BFCache 恢复要求重新载入；冲突冻结交互并提供当前页导出，导入同样校验快照。无 Web Locks 时停止写入并提示升级，不增加新数据库、不改存储键。词库与后台未变。
...
- 当前生产验收以本页顶部 v2.19.1 回读为准；后续发布仍需重新 fetch、验收并回读，不能只引用历史记录。
