...
2026-09-05 从 origin/main `0db8f71` 建立 `codex/learning-system-audit-20260905`，完成 v2.18.1–4 存储保护、永久错词/重点统计、每日固定题单及到期/加练排期审计。用户随后授权统一两条主线并发布；再次 fetch/rebase 确认基准未变化。当前 v2.19.0 为待发布验收版本，生产结果在实际发布回读后补记，不以历史记录代替。前轮详见 [自检报告](AUDIT-2026-09-05.md)。
...
- 本地当前为 v2.19.0：统一两条主线四选一与回炉，并包含前轮永久错词档案、主动赦免、独立重点维度、固定日任务及到期复习修复。
