...
发布准备：三个入口版本与缓存已统一，807 和易混刷词全轮及故障测试最后复验通过；发版前重新 fetch origin，禁止 force push。Cloudflare 后台没有任何代码变更，本轮只需 Pages 发布。

当前待发布候选：Listening `v2.20.0`、Confusions `v1.2.0`、807 `v1.1.0`。807 分支已经整合，运行时只读站内固定词表与 MP3，不再调用第三方词表或系统 TTS。三个独立模块记录均接入备份与写入保护，未增加后台服务。完整发布结果尚待 Pages 与生产回读，不能将以下本地验证视作已上线。
