...
<!-- VOCAB_STATUS_START -->
- 训练端程序版本：`v2.16.0`（当前分支候选，尚未发布）
- 后台程序版本：`v1.0.0`
- 正式词库：758 张主卡；听写 274 项；识词 537 项
- 个人错词：123 条；基础词覆盖：8 条；已停用：0 条
- 最后自动词库同步：2026-09-01，GitHub Issue #10
<!-- VOCAB_STATUS_END -->

- 最后完成线上运行态验收的功能提交：`a46ce00`（Listening `v2.15.1` / Confusions `v1.1.0`）
- 当前分支待发布：Listening `v2.16.0`。识义改为“认识 / 模糊 / 不认识”先自评后揭义；回炉间隔延长至 8–20 题，并在队尾间隔不足时转入高频复习；6 个无独立义的规则词形已归并到原形，`end / purpose / aim` 释义已按语境区分。
...
- Listening `v2.16.0` 仍是本地候选版，未推送、未发布；除这个候选版外没有其他已确认的技术待办。
