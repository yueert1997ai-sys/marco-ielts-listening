...
<!-- VOCAB_STATUS_START -->
- 训练端程序版本：`v2.15.1`
- 易混词程序版本：`v1.0.1`（当前分支候选，尚未完成线上回读）
- 后台程序版本：`v1.0.0`
...
- 最后完成线上运行态验收的功能提交：`72b2661`（Listening `v2.15.1` / Confusions `v1.0.0`）
...
- 当前分支待发布：Confusions `v1.0.1`。学习阶段的词义、Chunk 匹配与组末回忆均改为每屏至少 4 词，补位词不计入核心组学习状态或冷测统计。
