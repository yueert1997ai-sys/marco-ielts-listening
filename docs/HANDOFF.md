...
- 训练端程序版本：`v2.11.2`
- 后台程序版本：`v1.0.0`
- 正式词库：691 张主卡；听写 272 项；识词 470 项
- 个人错词：49 条；基础词覆盖：5 条；已停用：0 条
- 最后自动词库同步：2026-08-27，GitHub Issue #2
<!-- VOCAB_STATUS_END -->

- 最后完成线上运行态验收的提交：`bbc244d`
- 训练端由 GitHub Pages 托管；后台由 Cloudflare Worker + D1 托管。
- 词库后台只管理正式词库，不同步手机浏览器里的训练进度和记忆曲线。

## 当前待发布改动

- 本地训练端已准备 `v2.11.3`：听写正确后展示中文义，混合训练切回听写时衔接手机键盘，识义选项展示词性，并在次级菜单新增只抽取真实错词的独立“错词专项”。训练节奏同时加强：真实错词识义必做二次确认，普通识义约 40% 抽查；任何答错、超时或“不会”会在 2–4 题后回炉，连续答对两次才结束本轮巩固，单项最多插入 4 次以防无限循环。
- 该版本尚未推送或部署；线上训练端仍是 `v2.11.2`。发布后需更新本节、上方训练端版本和“最后完成线上运行态验收的提交”。
...
- 除本页“当前待发布改动”外，没有其他确认中的产品或技术待办；新需求以 GitHub 最新状态为起点重新评估。
