...
手机端 IELTS Listening 基础词训练：每天 25 个听音拼写 + 25 个快速看义，支持分页浏览、全词发音、重点词、记忆阶段和跨端错词同步。首页另有独立的“方位检测”：标准模式用 10 道、2 秒限时题训练八方位，困难模式只考四个 45° 方位并要求 1 秒内作答；两种模式都不影响每日词汇进度。
